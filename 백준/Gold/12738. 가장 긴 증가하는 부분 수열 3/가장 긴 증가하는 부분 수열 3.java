import java.util.*;
import java.io.*;

class Pair {
	int value;
	int index;
	
	Pair(int value, int index) {
		this.value = value;
		this.index = index;
	}
}

public class Main {
	static final int ROOT = 1;
	static int N; // 수열의 크기
	static int[] A; // 원본 수열
	static Pair[] pairs; // 원본 수열에서의 인덱스를 정렬 후에도 유지하기 위해 필요
	static int[] tree; // 인덱스 트리
	static int[] dp; // 수열에 동일한 값이 있을 경우 서로 영향을 주지 않고 쿼리하기 위해 필요
	
	public static void main(String[] args) throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
		StringTokenizer st = null;
		
		N = Integer.parseInt(br.readLine()); // 수열의 크기
		
		A = new int[N];
		pairs = new Pair[N];
		tree = new int[getTreeSize(N)];
		dp = new int[N];
		st = new StringTokenizer(br.readLine());
		
		// 수열 저장
		for (int i = 0; i < N; i++) {
			int value = Integer.parseInt(st.nextToken());
			A[i] = value;
			pairs[i] = new Pair(value, i);
		}
		
		// 값에 대해 오름차순 정렬
		Arrays.sort(pairs, (p1, p2) -> (Integer.compare(p1.value, p2.value)));
		
		int i = 0;
		
		while (i < N) {
			int j = getUpperBound(pairs[i].value); // 현재 값을 초과하는 값이 처음 발견되는 인덱스
			
			// 동일한 값에 대해 쿼리
			for (int k = i; k < j; k++) {
				int index = pairs[k].index;
				dp[index] = query(ROOT, 0, N - 1, 0, index - 1) + 1;
			}
			
			// 트리 업데이트
			for (int k = i; k < j; k++) {
				int index = pairs[k].index;
				update(ROOT, 0, N - 1, index, dp[index]);
			}
			
			i = j;
		}
		
		bw.write(String.valueOf(query(ROOT, 0, N - 1, 0, N - 1)));
		bw.flush();
		
		br.close();
		bw.close();
	}
	
	static int getTreeSize(int N) {
		int height = (int) Math.ceil(Math.log(N) / Math.log(2));
		int treeSize = (1 << (height + 1));
		return treeSize;
	}
	
	static int getMiddle(int start, int end) {
		return start + (end - start) / 2;
	}
	
	// 정렬된 배열에서 처음으로 target보다 큰 값을 갖는 인덱스 반환
	static int getUpperBound(int target) {
		int left = 0;
		int right = N;
		
		while (left < right) {
			int middle = getMiddle(left, right);
			
			if (pairs[middle].value <= target) {
				left = middle + 1;
			} else {
				right = middle;
			}
		}
		
		return left;
	}
	
	static int query(int node, int start, int end, int left, int right) {
		if (end < left || right < start) return 0;
		if (left <= start && end <= right) return tree[node];
		
		int middle = getMiddle(start, end);
		
		int leftResult = query(node * 2, start, middle, left, right);
		int rightResult = query(node * 2 + 1, middle + 1, end, left, right);
		
		return Math.max(leftResult, rightResult);
	}
	
	static void update(int node, int start, int end, int index, int value) {
		if (index < start || end < index) return;
		
		if (start == end) {
			tree[node] = value;
			return;
		}
		
		int middle = getMiddle(start, end);
		
		update(node * 2, start, middle, index, value);
		update(node * 2 + 1, middle + 1, end, index, value);
		
		tree[node] = Math.max(tree[node * 2], tree[node * 2 + 1]);
	}
}